(function initNexusAuthoritativeOutcomeRenderer(globalScope) {
  ...
  class NexusAuthoritativeOutcomeRenderer {
    ...
    async render(outcome) {
      ...
      const adapter = this.adapters.get(outcome.workspace);
      if (!adapter) throw new Error(`No passive renderer is registered for ${outcome.workspace}.`);
  ...
  function validateOutcome(outcome) {
    if (outcome?.schema !== "nexus.workspace-outcome.v1") throw new Error("A typed authoritative workspace outcome is required.");
