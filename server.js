Warning: truncated output (original token count: 692067)
... 1719689 bytes omitted ...
...
function smartNextActions(db, user, providers = runtimeProviders(db)) {
  ...
  const providerOk = id => ["connected", "ready"].includes(providers.find(item => item.id === id)?.status);
  const { country, route } = activeContext(db);
  const activeCourse = (db.courses || []).find(course =>…62152 tokens truncated…(/^\/api\/nexus\/provider-pathways\/([^/]+)\/route$/);
  ...
  if (url.pathname === "/api/nexus/knowledge/source-policy" && req.method === "GET") {
    return send(res, 200, { ok: true, categories: nexusKnowledgeSourceList(), rules: ["No fake URLs", "No fabricated source names", "Disabled mode returns zero citations", "Built-in guidance is labeled as built-in", "Retrieved answers must cite retrieved sources"] });
  }

  if (url.pathname === "/api/nexus/knowledge/readiness" && req.method === "GET") {
  ...
  const nexusKnowledgeHistoryDetailMatch = url.pathname.match(/^\/api\/nexus\/knowledge\/history\/([^/]+)$/);
  ...
  if (url.pathname === "/api/nexus/knowledge/classify" && req.method === "POST") {
    const body = await readBody(req);
    const classification = nexusKnowledgeClassifier(body.question || body.command || body.query || "", body.context || {});
    addNexusPilotAuditEvent(db, "knowledge_classification_completed", {
      actor: user?.name || "Standard User",
      role: user?.role || "Standard User",
      mode: classification.category,
      description: `Knowledge classification completed for ${classification.categoryLabel}. Retrieval needed: ${classification.retrievalNeeded ? "yes" : "no"}.`
    });
    await writeDb(db);
    return send(res, 200, { ok: true, classification, audit: db.nexusPilotAuditEvents[0] });
  }
  ...
  if (url.pathname === "/api/nexus/knowledge/attach-to-record" && req.method === "POST") {
    const result = nexusKnowledgeAttachToRecord(db, await readBody(req), user);
    if (!result.ok) return send(res, 404, result);
    await writeDb(db);
    return send(res, 200, result);
  }

  if (url.pathname === "/api/nexus/knowledge/prepare-review-summary" && req.method === "POST") {
    const result = nexusKnowledgePrepareReviewSummary(db, await readBody(req), user);
    await writeDb(db);
    return send(res, 200, result);
  }
  ...
  if (url.pathname === "/api/nexus/health-evidence/medication-pharmacy" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildMedicationPharmacyEvidencePacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/laboratory-diagnostic" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildLaboratoryDiagnosticEvidencePacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/consent-rights" && req.method === "POST") {
    const body = await readBody(req);
  ...
  if (url.pathname === "/api/nexus/health-evidence/accessibility-localization" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildAccessibilityLocalizationGovernancePacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/communications-follow-up" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildHealthCommunicationsFollowUpPacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/monitoring" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildHealthModelSourceMonitoringPacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/regulatory-assessment" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildHealthRegulatoryAssessmentPacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/security-privacy-adversarial" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildHealthSecurityPrivacyAdversarialPacket(body?.text || body?.command || "", body?.context || {}));
  }

  if (url.pathname === "/api/nexus/health-evidence/capability-status" && req.method === "POST") {
    const body = await readBody(req);
    return send(res, 200, nexusEnterpriseHealthEvidenceTrust.buildHealthGenesisCapabilityStatusPacket(body?.text || body?.command || "", body?.context || {}));
  }
  ...
  if (url.pathname === "/api/native/voice-architecture" && req.method === "GET") {
    return send(res, 200, {
      ...
      realtimeVoice: {
        ...
        operationalMode: "realtime-tools",
        canExecuteWorkflows: true,
        toolEndpoint: "/api/voice/realtime/tool",
        toolName: "nexus_capability_router",
        safety: "Tools route through Nexus response envelopes and existing confirmation gates.",
        ready: Boolean(process.env.OPENAI_API_KEY),
        model: openAiRealtimeModel(),
        voice: openAiRealtimeVoice()
      },
...
<Response>
  ...
  if (url.pathname === "/api/voice/elevenlabs/status" && req.method === "GET") {
    return send(res, 410, {
      error: "ElevenLabs Genesis runtime has been removed.",
  ...
  if ((url.pathname === "/api/voice/runtime/tool" || url.pathname === "/api/voice/elevenlabs/tool") && req.method === "POST") {
    return send(res, 410, {
      ok: false,
      error: "Legacy voice tool gateways have been removed. Use /api/voice/realtime/tool.",
      category: "gateway-removed",
