Warning: truncated output (original token count: 695021)
... 1731505 bytes omitted ...
...
function smartNextActions(db, user, providers = runtimeProviders(db)) {
  ...
  ensureAiProfile(db.profile);
  ensureOperationsProfile(db.profile);
  const providerOk = id => ["connected", "ready"].includ…62152 tokens truncated…PilotAuditEvents[0] });
  }
