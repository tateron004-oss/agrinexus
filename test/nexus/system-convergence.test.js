...
test("production exposes one authoritative durable runtime for all 19 workspaces", () => {
  const server = read("server.js");
  const factory = read("nexus/runtime/create-runtime.js");
  const blueprint = read("render.yaml");
  const workspaces = defaultApplicationManifests();

  assert.equal(workspaces.length, 19);
  assert.equal(new Set(workspaces.map(item => item.applicationId)).size, 19);
