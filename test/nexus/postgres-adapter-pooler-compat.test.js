...
test("PostgresAdapter never puts statement_timeout in the Pool startup config (PgBouncer rejects unknown startup parameters)", () => {
  const adapter = new PostgresAdapter({ connectionString: "postgres://localhost:1/db", statementTimeoutMs: 45000 });
  try {
...
test("PostgresAdapter's connect handler issues the configured timeout value and tolerates a query failure", async () => {
  ...
  console.error = () => {};
  try {
    const adapter = new PostgresAdapter({ connectionString: "postgres://localhost:1/db", statementTimeoutMs: 12345 });
    try {
...
test("PostgresAdapter falls back to a 60s default when statementTimeoutMs is missing or invalid", () => {
  const adapter = new PostgresAdapter({ connectionString: "postgres://localhost:1/db", statementTimeoutMs: "not-a-number" });
  const queries = [];
  try {
