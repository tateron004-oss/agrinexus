...
const fixturePath = path.join(root, "fixtures", "nexus", "workforce-mode-feature-flags.json");

const protectedFields = Array.from(PROTECTED_WORKFORCE_MODE_FLAG_FIELDS);

function loadWorkforceModeFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateWorkforceModeFlagFixtures(fixtures = loadWorkforceModeFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeWorkforceModeFeatureFlagState(fixture && fixture.input);
    const visible = isWorkforceModeVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateWorkforceModeFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-ad3-workforce-mode-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-ad3-workforce-mode-flag-contract-harness] passed (${result.count} fixtures)`);
}
