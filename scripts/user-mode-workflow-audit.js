...
assert(html.includes("/app.js?v=nexus-behavior-109"), "Index must force browsers to load current User-mode workflow JS");
assert(html.includes("/styles.css?v=nexus-behavior-109"), "Index must force browsers to load current User-mode workflow CSS");
assert(sw.includes('CACHE_NAME = "agrinexus-pwa-v89"'), "Service worker cache must be bumped after User-mode workflow fixes");
