...
hasAll(app, [
  ...
  "function updateStreamingVoicePartial",
  "function scheduleFinalVoiceCommand",
  "function registerNativeVoiceSession",
...
assert(app.includes("function isLikelySideConversationWithoutNexusCommand"), "Nexus must detect side conversation before guessing at a command");
assert(app.includes("function pauseNexusForSideConversation"), "Nexus must pause silently when it hears multiple people without a clear command");
assert(app.includes("I heard people talking, so I paused. Say Nexus when you want me."), "Side-conversation pause must explain briefly and wait for a wake phrase");
assert(app.includes("function conversationMode2Status"), "Nexus needs a named Conversation Mode 2.0 status model");
...
assert(app.includes("function normalizeRuralSpeechAliases") && app.includes("function ruralCommunicationBridge"), "Nexus needs a rural communication bridge before voice routing");
assert(app.includes("dawa") && app.includes("shamba") && app.includes("soko") && app.includes("kazi") && app.includes("ramani"), "Rural communication bridge must understand practical regional words for medicine, farm, market, work, and map");
assert(app.includes("I think you mean ${intentLabels[profile.intent]}"), "Rural clarification must repeat the likely meaning before asking one question");
...
assert(server.includes("conversation-resilience-v2") && server.includes("mixed-language-meaning-bridge") && server.includes("repair-and-confirm-before-action"), "Conversation resilience must include v2 profiles for mixed language and misheard corrections");
assert(server.includes("supportMoves") && server.includes("nextQuestionByModule") && server.includes("pause-for-clear-wake-phrase"), "Conversation resilience must provide support moves, module-specific questions, and background-speech pause behavior");
...
assert(app.includes("function answerPendingNexusQuestion"), "Nexus must answer a pending question before routing a new command");
...
assert(app.indexOf("unified-brain-platform-explain") < app.indexOf("handleNexusAdaptiveLearningCommand(command || localized || rawCommand)"), "Platform explanation must run before adaptive/workflow interception");
assert(app.indexOf("unified-brain-platform-explain") < app.indexOf("if (pendingWorkflow && visibleInlineWorkflow)"), "Platform explanation must run before open workflow handling");
...
assert(app.includes("function openTelehealthCaptionsNow") && app.includes('intent.workflow === "health" && intent.action === "caption"') && app.includes('intent === "conversation.telehealth_captions"'), "Telehealth captions must route into guided health caption support, not a silent workflow card");
assert(app.includes("Who needs captions first?") && app.includes("Nexus started guided telehealth caption relay"), "Telehealth captions need a guided response and next question");
assert(html.includes("nexus-behavior-279"), "Index must force browsers to load Nexus behavior CSS");
assert(html.includes("nexus-behavior-279"), "Index must force browsers to load Nexus behavior JS");
...
assert(app.includes("World_Street_Map") && app.includes("Operational map") && app.includes("OpenStreetMap") && app.includes("Humanitarian street map"), "Maps must default to real scalable tile layers with fallback choices");
assert(!app.includes("Regional Africa operations view") && !app.includes("Regional Africa health access view"), "Maps must not use decorative regional overlay labels that make the map look cartoonish");
...
assert(app.includes("recognition.interimResults = streamingVoiceEnabled() || chromeVoiceRuntimeReady()") && app.includes("const keepStreamingOpen = streamingVoiceEnabled()"), "Voice recognition must stay open for streaming/native-style sessions");
...
assert(app.includes("function ruralAfricaConversationStyle") && app.includes("pan-african-rural-plain-language") && app.includes("Baby hot... no doctor"), "Nexus needs a pan-African incomplete-sentence conversation style for rural users");
assert(app.includes("function ruralKenyaCommunicationStyle") && app.includes("Twende hatua moja") && app.includes("no fake accent"), "Nexus needs a respectful rural Kenya/Kiswahili communication style without caricature");
...
assert(app.includes("I heard you need medicine. I can guide you step by step."), "User medicine voice command should acknowledge, explain, and guide pharmacy/mobile clinic support before provider/intake routing");
assert(app.includes("I heard you need a clinic or mobile clinic."), "User clinic-near-me voice command should explain and guide clinic/mobile clinic/map support before provider routing");
assert(app.includes("I am not a doctor and this is not a diagnosis"), "Doctor and health voice guidance must preserve the non-diagnosis safety boundary");
