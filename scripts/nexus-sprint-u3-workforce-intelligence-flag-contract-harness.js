...
const fixturePath = path.join(root, "fixtures", "nexus", "workforce-intelligence-feature-flags.json");

const protectedFields = Array.from(PROTECTED_WORKFORCE_INTELLIGENCE_FLAG_FIELDS);

function loadWorkforceIntelligenceFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateWorkforceIntelligenceFlagFixtures(fixtures = loadWorkforceIntelligenceFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeWorkforceIntelligenceFeatureFlagState(fixture && fixture.input);
    const visible = isWorkforceIntelligenceVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateWorkforceIntelligenceFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-u3-workforce-intelligence-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-u3-workforce-intelligence-flag-contract-harness] passed (${result.count} fixtures)`);
}
