...
const fixturePath = path.join(root, "fixtures", "nexus", "telehealth-mode-feature-flags.json");

const protectedFields = Array.from(PROTECTED_TELEHEALTH_MODE_FLAG_FIELDS);

function loadTelehealthModeFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateTelehealthModeFlagFixtures(fixtures = loadTelehealthModeFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeTelehealthModeFeatureFlagState(fixture && fixture.input);
    const visible = isTelehealthModeVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateTelehealthModeFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-z3-telehealth-mode-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-z3-telehealth-mode-flag-contract-harness] passed (${result.count} fixtures)`);
}
