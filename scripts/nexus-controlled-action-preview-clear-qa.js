...
assert.match(backendCommandBody, /if \(!visibleControlledActionPreviewReadiness\)[\s\S]*paintLocalLevelOneSuggestionForSimpleUserIntent\(\{ type: "direct" \}, command\)/, "backend answers without metadata should allow low-risk local preview fallback");
assert.match(utilityCommandBody, /if \(!visibleControlledActionPreviewReadiness\)[\s\S]*paintLocalLevelOneSuggestionForSimpleUserIntent\(\{ type: "direct" \}, command\)/, "utility answers without metadata should allow low-risk local preview fallback");
