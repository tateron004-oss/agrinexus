...
assert(html.includes("styles.css?v=nexus-behavior-170"), "Index must force browsers to load the latest Nexus behavior shell");
assert(html.includes("app.js?v=nexus-behavior-170"), "Index must force browsers to load the latest Nexus behavior code");
...
assert(sw.includes("agrinexus-pwa-v150"), "Service worker cache must refresh the installed app after native voice bridge updates");
...
assert(app.includes("Operational map") && app.includes("Satellite imagery") && app.includes("Street map"), "User maps need real operational, satellite, and street map views");
...
assert(styles.includes(".map-grid-label") && styles.includes(".global-map-control"), "Gridline labels and global map control need visible styling");
...
assert(sw.includes("BUILD_VERSION = \"nexus-behavior-170\""), "Service worker must know the current app build version");
