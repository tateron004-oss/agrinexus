...
assert(html.includes("styles.css?v=nexus-behavior-73"), "Index must force browsers to load the latest Nexus behavior shell");
assert(html.includes("app.js?v=nexus-behavior-73"), "Index must force browsers to load the latest Nexus behavior code");
assert(sw.includes("agrinexus-pwa-v53"), "Service worker cache must refresh the installed app after native voice bridge updates");
...
assert(app.includes('$("#workflowTitle").textContent = translateText(config.title || "Workflow")'), "Workflow modal must show the real workflow title");
assert(app.includes('$("#workflowSummary").textContent = translateText(config.summary || "Review this workflow and confirm when ready.")'), "Workflow modal must show the real workflow summary");
