...
assert(app.includes("recognition.continuous = voiceFirstMode || streamingVoiceEnabled() || chromeVoiceRuntimeReady()"), "Voice-first mode should keep listening across conversation turns");
...
assert(server.includes("function directVendorProviderStatus"), "Backend must recognize direct vendor credentials without needing a custom webhook");
assert(server.includes("SUPABASE_SERVICE_ROLE_KEY"), "Production auth direct vendor slots must include Supabase");
assert(server.includes("RESEND_API_KEY"), "Email delivery direct vendor slots must include Resend");
assert(server.includes("PAYSTACK_SECRET_KEY"), "Trade/payment direct vendor slots must include Paystack");
assert(server.includes("FLUTTERWAVE_SECRET_KEY"), "Trade/payment direct vendor slots must include Flutterwave");
assert(server.includes("SENTINEL_HUB_CLIENT_SECRET"), "Drone/satellite provider depth must include Sentinel Hub");
assert(server.includes("production.provider_activation"), "Nexus must answer provider activation and missing-key questions");
...
assert(html.includes("styles.css?v=nexus-behavior-275"), "Index must force browsers to load the latest Nexus behavior shell");
assert(html.includes("app.js?v=nexus-behavior-275"), "Index must force browsers to load the latest Nexus behavior code");
...
assert(sw.includes("agrinexus-pwa-v255"), "Service worker cache must refresh the installed app after native voice bridge updates");
...
assert(app.includes("handleVoiceCommand(finalCommand, { source: \"voice\", turnToken })"), "Voice recognition should mark speech input so side conversation guard can apply");
assert(app.includes("function beginNexusVoiceTurn") && app.includes("function ignoreStaleNexusTurn"), "Voice recognition should suppress stale previous responses when a newer command arrives");
...
assert(sw.includes("BUILD_VERSION = \"nexus-behavior-275\""), "Service worker must know the current app build version");
