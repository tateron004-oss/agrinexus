...
assert(html.includes("styles.css?v=nexus-behavior-257"), "Index must force browsers to load the latest Nexus behavior shell");
assert(html.includes("app.js?v=nexus-behavior-257"), "Index must force browsers to load the latest Nexus behavior code");
...
assert(sw.includes("agrinexus-pwa-v237"), "Service worker cache must refresh the installed app after native voice bridge updates");
...
assert(sw.includes("BUILD_VERSION = \"nexus-behavior-257\""), "Service worker must know the current app build version");
