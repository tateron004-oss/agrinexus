...
assert(html.includes("styles.css?v=nexus-behavior-236"), "Index must force browsers to load the latest Nexus behavior shell");
assert(html.includes("app.js?v=nexus-behavior-236"), "Index must force browsers to load the latest Nexus behavior code");
...
assert(sw.includes("agrinexus-pwa-v216"), "Service worker cache must refresh the installed app after native voice bridge updates");
...
assert(app.includes("handleVoiceCommand(command, { source: \"voice\", turnToken })"), "Voice recognition should mark speech input so side conversation guard can apply");
...
assert(sw.includes("BUILD_VERSION = \"nexus-behavior-236\""), "Service worker must know the current app build version");
