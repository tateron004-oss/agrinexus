...
assert(html.includes("styles.css?v=grandma-voice-routes-48"), "Index must force browsers to load the latest Grandma Mode voice routing shell");
assert(html.includes("app.js?v=grandma-voice-routes-61"), "Index must force browsers to load the latest Grandma Mode voice routing code");
assert(sw.includes("agrinexus-pwa-v40"), "Service worker cache must refresh the installed app after Grandma Mode voice routing updates");
...
assert(app.includes('classList.toggle("grandma-workflow"'), "User mode workflows need simplified Grandma-safe modal state");
assert(app.includes('const grandmaMode = experienceMode === "user"'), "Confirmed workflows must stay in Grandma Mode after Yes");
assert(app.includes('Choose another button when ready'), "Grandma Mode post-confirm response should be short and contained");
assert(app.includes('Do you want Nexus to do this now?'), "Grandma Mode workflow modal needs short plain-language confirmation copy");
...
assert(styles.includes(".grandma-workflow .modal-close") && styles.includes("display: none !important"), "Grandma Mode should hide the wrapping close button");
assert(styles.includes("grid-template-rows: auto 1fr auto"), "Grandma Mode modal should fill the app screen cleanly");
