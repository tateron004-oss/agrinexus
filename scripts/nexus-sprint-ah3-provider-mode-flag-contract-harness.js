...
const fixturePath = path.join(root, "fixtures", "nexus", "provider-mode-feature-flags.json");

const protectedFields = Array.from(PROTECTED_PROVIDER_MODE_FLAG_FIELDS);

function loadProviderModeFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateProviderModeFlagFixtures(fixtures = loadProviderModeFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeProviderModeFeatureFlagState(fixture && fixture.input);
    const visible = isProviderModeVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateProviderModeFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-ah3-provider-mode-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-ah3-provider-mode-flag-contract-harness] passed (${result.count} fixtures)`);
}
