...
const fixturePath = path.join(root, "fixtures", "nexus", "africa-regional-deployment-mode-feature-flags.json");

const protectedFields = Array.from(PROTECTED_AFRICA_REGIONAL_DEPLOYMENT_MODE_FLAG_FIELDS);

function loadAfricaRegionalDeploymentModeFlagFixtures() {
...
function validateAfricaRegionalDeploymentModeFlagFixtures(fixtures = loadAfricaRegionalDeploymentModeFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeAfricaRegionalDeploymentModeFeatureFlagState(expandFixtureInput(fixture && fixture.input));
    const visible = isAfricaRegionalDeploymentModeVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateAfricaRegionalDeploymentModeFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-ak3-africa-regional-deployment-mode-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-ak3-africa-regional-deployment-mode-flag-contract-harness] passed (${result.count} fixtures)`);
}
