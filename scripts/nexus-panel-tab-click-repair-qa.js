...
[
  ["nexus-behavior-426", "app build version", "server build version"],
  ["agrinexus-pwa-v371", "app cache version", "server cache version"]
].forEach(([version, appLabel, serverLabel]) => {
  includes(app, version, `${appLabel} ${version}`);
  includes(server, version, `${serverLabel} ${version}`);
});
includes(sw, 'CACHE_NAME = "agrinexus-pwa-v371"', "service worker cache bump");
includes(sw, 'BUILD_VERSION = "nexus-behavior-426"', "service worker build bump");
includes(index, "/styles.css?v=nexus-behavior-426", "stylesheet cache bust");
includes(index, "/app.js?v=nexus-behavior-426", "app cache bust");
