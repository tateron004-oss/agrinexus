...
[
  ["nexus-behavior-464", "app build version", "server build version"],
  ["agrinexus-pwa-v409", "app cache version", "server cache version"]
].forEach(([version, appLabel, serverLabel]) => {
  includes(app, version, `${appLabel} ${version}`);
  includes(server, version, `${serverLabel} ${version}`);
});
includes(sw, 'CACHE_NAME = "agrinexus-pwa-v409"', "service worker cache bump");
includes(sw, 'BUILD_VERSION = "nexus-behavior-464"', "service worker build bump");
