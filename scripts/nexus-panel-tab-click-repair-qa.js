...
[
  ["nexus-behavior-487", "app build version", "server build version"],
  ["agrinexus-pwa-v432", "app cache version", "server cache version"]
].forEach(([version, appLabel, serverLabel]) => {
  includes(app, version, `${appLabel} ${version}`);
  includes(server, version, `${serverLabel} ${version}`);
});
includes(sw, 'CACHE_NAME = "agrinexus-pwa-v432"', "service worker cache bump");
includes(sw, 'BUILD_VERSION = "nexus-behavior-487"', "service worker build bump");
includes(index, "/styles.css?v=nexus-behavior-487", "stylesheet cache bust");
includes(index, "/app.js?v=nexus-behavior-487", "app cache bust");
