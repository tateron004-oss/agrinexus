const fs = require("node:fs");
const path = require("node:path");
const {
  normalizeTrustFraudRiskDetectionFeatureFlagState,
  isTrustFraudRiskDetectionVisibleFeatureEnabled,
  PROTECTED_TRUST_FRAUD_RISK_DETECTION_FLAG_FIELDS
} = require("../public/nexus-trust-fraud-risk-detection-feature-flag.js");

const root = path.resolve(__dirname, "..");
const fixturePath = path.join(root, "fixtures", "nexus", "trust-fraud-risk-detection-feature-flags.json");

const protectedFields = Array.from(PROTECTED_TRUST_FRAUD_RISK_DETECTION_FLAG_FIELDS);

function loadTrustFraudRiskDetectionFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateTrustFraudRiskDetectionFlagFixtures(fixtures = loadTrustFraudRiskDetectionFlagFixtures()) {
  if (!Array.isArray(fixtures)) {
    return { ok: false, count: 0, failures: ["fixture root must be an array"] };
  }

  const failures = [];
  fixtures.forEach((fixture, index) => {
    const label = fixture && fixture.fixtureId ? fixture.fixtureId : `fixture-${index}`;
    const normalized = normalizeTrustFraudRiskDetectionFeatureFlagState(fixture && fixture.input);
    const visible = isTrustFraudRiskDetectionVisibleFeatureEnabled(normalized);
    const expected = fixture && fixture.expected ? fixture.expected : {};

    for (const [field, expectedValue] of Object.entries(expected)) {
      const actualValue = field === "isVisible" ? visible : normalized[field];
      if (actualValue !== expectedValue) {
        failures.push(`${label} expected ${field}=${expectedValue} but received ${actualValue}`);
      }
    }

    for (const field of protectedFields) {
      if (normalized[field] !== false) {
        failures.push(`${label} must keep ${field}=false`);
      }
    }

    if (normalized.noExecution !== true) {
      failures.push(`${label} must keep noExecution=true`);
    }
  });

  return { ok: failures.length === 0, count: fixtures.length, failures };
}

if (require.main === module) {
  const result = validateTrustFraudRiskDetectionFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-w3-trust-fraud-risk-detection-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-w3-trust-fraud-risk-detection-flag-contract-harness] passed (${result.count} fixtures)`);
}

module.exports = {
  fixturePath,
  protectedFields,
  loadTrustFraudRiskDetectionFlagFixtures,
  validateTrustFraudRiskDetectionFlagFixtures
};
