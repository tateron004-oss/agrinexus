...
assert(/if \(handleJarvisStyleStandardUserSafetyResponse\(command\)\) return;\s+(?:if \(handleNexusSimulationCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusMapNavigationHandoffCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusInternalNavigationCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusLocalDraftMessageCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusCallPreparationCaptionCommand\(command\)\) return;\s+)?void handleVoiceCommand\(command\);/.test(app), "caption Send should use Jarvis-style handling before handleVoiceCommand");
