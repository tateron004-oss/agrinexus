...
assert(/if \(handleJarvisStyleStandardUserSafetyResponse\(command\)\) return;\s+(?:if \(handleNexusSimulationCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusInternalNavigationCaptionCommand\(command\)\) return;\s+)?void handleVoiceCommand\(command\);/.test(app), "caption Send should use Jarvis-style handling before handleVoiceCommand");
assert(/if \(handleJarvisStyleStandardUserSafetyResponse\(command\)\) return;\s+(?:if \(handleNexusSimulationCaptionCommand\(command\)\) return;\s+)?(?:if \(handleNexusInternalNavigationCaptionCommand\(command\)\) return;\s+)?void handleVoiceCommand\(command\);/.test(app), "caption Enter should use Jarvis-style handling before handleVoiceCommand");
