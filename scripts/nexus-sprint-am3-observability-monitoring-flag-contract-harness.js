...
const fixturePath = path.join(root, "fixtures", "nexus", "observability-monitoring-feature-flags.json");

const protectedFields = Array.from(PROTECTED_OBSERVABILITY_MONITORING_FLAG_FIELDS);

function loadObservabilityMonitoringFlagFixtures() {
...
function validateObservabilityMonitoringFlagFixtures(fixtures = loadObservabilityMonitoringFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeObservabilityMonitoringFeatureFlagState(expandFixtureInput(fixture && fixture.input));
    const visible = isObservabilityMonitoringVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateObservabilityMonitoringFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-am3-observability-monitoring-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-am3-observability-monitoring-flag-contract-harness] passed (${result.count} fixtures)`);
}
