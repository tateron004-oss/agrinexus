...
const fixturePath = path.join(root, "fixtures", "nexus", "user-profile-feature-flags.json");
...
function loadUserProfileFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateUserProfileFlagFixtures(fixtures = loadUserProfileFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeUserProfileFeatureFlagState(fixture && fixture.input);
    const visible = isUserProfileVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateUserProfileFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-j3-user-profile-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-j3-user-profile-flag-contract-harness] passed (${result.count} fixtures)`);
}
