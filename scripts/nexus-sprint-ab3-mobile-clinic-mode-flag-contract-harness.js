...
const fixturePath = path.join(root, "fixtures", "nexus", "mobile-clinic-mode-feature-flags.json");

const protectedFields = Array.from(PROTECTED_MOBILE_CLINIC_MODE_FLAG_FIELDS);

function loadMobileClinicModeFlagFixtures() {
  return JSON.parse(fs.readFileSync(fixturePath, "utf8"));
}

function validateMobileClinicModeFlagFixtures(fixtures = loadMobileClinicModeFlagFixtures()) {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const normalized = normalizeMobileClinicModeFeatureFlagState(fixture && fixture.input);
    const visible = isMobileClinicModeVisibleFeatureEnabled(normalized);
...
if (require.main === module) {
  const result = validateMobileClinicModeFlagFixtures();
  if (!result.ok) {
    console.error("[nexus-sprint-ab3-mobile-clinic-mode-flag-contract-harness] failed");
    result.failures.forEach(failure => console.error(`- ${failure}`));
    process.exit(1);
  }

  console.log(`[nexus-sprint-ab3-mobile-clinic-mode-flag-contract-harness] passed (${result.count} fixtures)`);
}
