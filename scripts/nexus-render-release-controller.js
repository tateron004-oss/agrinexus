  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (response.status >= 500 && body?.releaseSha === releaseSha && body?.stage) {
        const failureCode = String(body?.code || "acceptance_failed").replace(/[^a-z0-9_-]/gi, "").slice(0, 64) || "acceptance_failed";
        throw Object.assign(new Error(`Production acceptance failed at ${body?.stage || "unknown-stage"} (${failureCode}).`), { code: failureCode, stage: body?.stage || null, failFast: true });
      }
...
module.exports = { CANONICAL_NEXUS_BASE_URL, CANONICAL_PROVIDER_BASE_URL, REQUIRED_WEB_ENV, HOSTED_PROVIDER_ENV, SHARED_PROVIDER_SECRET_KEYS, createClient, resolveUniqueService, validateService, reconcileServiceConfiguration, resolveOrProvisionDatabase, installEnvValue, readServiceEnv, ensureGeneratedEnvSecret, ensureSharedEnvSecret, installHostedProviderContract, canonicalToolProviders, installCanonicalToolProviders, provisionBackgroundWorker, resolveOrProvisionWorker, resolveReusableDeploy, fetchDeployDiagnostics, sanitizeDiagnostic, deployExactSha, waitForAcceptanceToken, run };
