...
function createControlApi(runtime) {
  return Object.freeze({
    async registerDevice(r) { permit(r.context,"devices:write"); const deviceId=req(r.body.deviceId,"Device ID"); let pushKeyCiphertext=null; if(r.body.pushSubscription){if(!runtime.deviceTokens)throw new NexusRuntimeError("device_token_key_missing","Push registration is unavailable because encrypted token storage is not configured.",503);pushKeyCiphertext=runtime.deviceTokens.encrypt(r.body.pushSubscription,`${r.context.tenantId}:${r.context.userId}:${deviceId}`);} return respond(()=>runtime.devices.register({deviceId,tenantId:r.context.tenantId,userId:r.context.userId,platform:req(r.body.platform,"Platform"),capabilities:r.body.capabilities||[],pushEndpoint:r.body.pushEndpoint,pushKeyCiphertext}),201); },
