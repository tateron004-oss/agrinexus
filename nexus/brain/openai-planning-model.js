  ...
  async plan(request) {
    const response = await this.fetchFn("https://api.openai.com/v1/responses", { method: "POST",
      ...
      body: JSON.stringify({ model: this.model,
        instructions: "Plan the user's open-ended goal using only the supplied application and tool catalog. Ask one clarification only when essential. Never claim execution. Return JSON matching the schema.",
