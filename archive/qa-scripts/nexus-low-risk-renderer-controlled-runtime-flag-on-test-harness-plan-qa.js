...
const docPath = path.join(root, "docs", "NEXUS_LOW_RISK_RENDERER_CONTROLLED_RUNTIME_FLAG_ON_TEST_HARNESS_PLAN.md");
assert(fs.existsSync(docPath), "docs/NEXUS_LOW_RISK_RENDERER_CONTROLLED_RUNTIME_FLAG_ON_TEST_HARNESS_PLAN.md must exist");
...
const doc = read("docs", "NEXUS_LOW_RISK_RENDERER_CONTROLLED_RUNTIME_FLAG_ON_TEST_HARNESS_PLAN.md");
...
for (const section of [
  ...
  assert(doc.includes(section), `Phase 12V doc must include section: ${section}`);
}

for (const term of [
  "Controlled Runtime Wiring Flag-On Test Harness Plan",
  "disabled by default",
  ...
  assert(doc.includes(term), `Phase 12V doc must include required safety language: ${term}`);
}
...
for (const fixture of [
  ...
  assert(doc.includes(fixture), `Phase 12V doc must document fixture scope: ${fixture}`);
}

assert.match(doc, /ready for \*\*Phase 12W - Controlled Runtime Flag-On Harness Static QA\*\*/i, "decision must name Phase 12W readiness");
assert.match(doc, /Phase 12W should add static QA/i, "recommendation must keep Phase 12W static-QA-first");
...
assert(packageJson.includes("\"qa:nexus-low-risk-renderer-controlled-runtime-flag-on-test-harness-plan\""), "package.json must expose qa:nexus-low-risk-renderer-controlled-runtime-flag-on-test-harness-plan");
assert(suite.includes("archive/qa-scripts/nexus-low-risk-renderer-controlled-runtime-flag-on-test-harness-plan-qa.js"), "nexus-workforce suite must include Phase 12V plan QA");
...
console.log("Nexus low-risk renderer controlled runtime flag-on test harness plan QA passed");
console.log("- Phase 12V plan, fixture scope, and safety language are documented");
console.log("- index.html remains unwired and no renderer UI container was introduced");
console.log("- Phase 12W is correctly recommended as static QA before implementation");
