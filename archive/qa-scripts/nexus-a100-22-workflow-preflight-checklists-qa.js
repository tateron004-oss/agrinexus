...
const sprint = require("../../server/nexus-a100-22-workflow-preflight-checklists.js");
...
function assertStaticSafety() {
  const source = read("server", "nexus-a100-22-workflow-preflight-checklists.js");
  const doc = read("docs", "NEXUS_A100_22_WORKFLOW_PREFLIGHT_CHECKLISTS.md");
  ...
  assert(exists("server", "nexus-a100-22-workflow-preflight-checklists.js"), "A100-22 module must exist.");
  assert(exists("docs", "NEXUS_A100_22_WORKFLOW_PREFLIGHT_CHECKLISTS.md"), "A100-22 documentation must exist.");
  assert(exists("archive", "qa-scripts", "nexus-a100-22-workflow-preflight-checklists-qa.js"), "A100-22 QA must exist.");
  if (22 === 1) {
  ...
  ].forEach(term => assert(source.includes(term), `A100-22 source must include ${term}.`));

  [
    "Workflow Preflight Checklists",
    "review-only",
  ...
  ].forEach(term => assert(doc.includes(term), `A100-22 documentation must include ${term}.`));

  [
    "nexus-a100-22-workflow-preflight-checklists",
    "createA100Sprint22Artifact"
  ].forEach(term => {
    assert(!app.includes(term), `public/app.js must not load A100-22 runtime term: ${term}.`);
    assert(!indexHtml.includes(term), `public/index.html must not load A100-22 runtime term: ${term}.`);
    assert(!server.includes(term), `server.js must not load A100-22 runtime term: ${term}.`);
  });
  ...
  ].forEach(term => assert(!source.includes(term), `A100-22 module must not introduce unsafe behavior: ${term}.`));

  assert.equal(pkg.scripts["qa:nexus-a100-22-workflow-preflight-checklists"], "node archive/qa-scripts/nexus-a100-22-workflow-preflight-checklists-qa.js", "A100-22 package QA alias must exist.");
  assert(qaSuite.includes("archive/qa-scripts/nexus-a100-22-workflow-preflight-checklists-qa.js"), "A100-22 QA must be wired into local-safe suites.");
}

function assertArtifacts() {
  {
    const artifact = sprint.createA100Sprint22Artifact({ prompt: "Prepare agriculture support.", lane: "agriculture" });
    assert.equal(sprint.isSafeA100Sprint22Artifact(artifact), true, "agriculture artifact must be safe.");
  ...
  {
    const artifact = sprint.createA100Sprint22Artifact({ prompt: "Prepare learning support.", lane: "learning" });
    assert.equal(sprint.isSafeA100Sprint22Artifact(artifact), true, "learning artifact must be safe.");
  ...
  {
    const artifact = sprint.createA100Sprint22Artifact({ prompt: "Prepare workforce support.", lane: "workforce" });
    assert.equal(sprint.isSafeA100Sprint22Artifact(artifact), true, "workforce artifact must be safe.");
  ...
  {
    const artifact = sprint.createA100Sprint22Artifact({ prompt: "Prepare marketplace support.", lane: "marketplace" });
    assert.equal(sprint.isSafeA100Sprint22Artifact(artifact), true, "marketplace artifact must be safe.");
  ...
  {
    const artifact = sprint.createA100Sprint22Artifact({ prompt: "Prepare health support.", lane: "health" });
    assert.equal(sprint.isSafeA100Sprint22Artifact(artifact), true, "health artifact must be safe.");
...
function runA100Sprint22Qa() {
  ...
  console.log("[nexus-a100-22-workflow-preflight-checklists-qa] passed");
}
