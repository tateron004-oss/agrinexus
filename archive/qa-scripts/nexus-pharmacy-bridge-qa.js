...
(async () => {
  ...
  assert((await provider.search({ query: "medication counseling" })).body.data.cards.length >= 1);
  assert((await provider.search({ query: "chronic care" })).body.data.cards.length >= 1);
  assert((await provider.search({ query: "diabetes" })).body.data.cards.length >= 1);
  assert((await provider.search({ query: "hypertension" })).body.data.cards.length >= 1);
  assert((await provider.search({ query: "low-cost medication resource" })).body.data.cards.length >= 1);
