...
function assertAlias(name, script) {
  assert(packageJson.scripts[name] === `node archive/qa-scripts/${script}`, `${name} package alias exists`);
...
function assertCacheResponsive() {
  assert(app.includes('const AGRINEXUS_BUILD_VERSION = "__NEXUS_RELEASE_SHA__"'), "app build version bumped");
  assert(app.includes('const AGRINEXUS_PWA_CACHE_VERSION = "agrinexus-pwa-__NEXUS_RELEASE_SHA__"'), "app cache version bumped");
  assert(server.includes("const AGRINEXUS_WEB_BUILD_VERSION = NEXUS_EFFECTIVE_RELEASE_SHA"), "server build uses immutable release SHA");
  assert(server.includes("const AGRINEXUS_PWA_CACHE_VERSION = `agrinexus-pwa-${NEXUS_EFFECTIVE_RELEASE_SHA}`"), "server cache uses immutable release SHA");
  assert(sw.includes('const CACHE_NAME = "agrinexus-pwa-__NEXUS_RELEASE_SHA__"'), "service worker cache version bumped");
