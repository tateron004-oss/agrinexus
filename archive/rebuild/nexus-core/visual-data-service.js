...
function createVisualDataService({
  ...
  const providerFetch = createProviderFetch({ fetchImpl, maxAttempts: weatherRetryAttempts, sleepImpl: wait });
  ...
  async function retrieveMetNorwayWeather(requested) {
    ...
    if (!place || place.lat == null || place.lon == null || !Number.isFinite(latitude) || !Number.isFinite(longitude) || Math.abs(latitude) > 90 || Math.abs(longitude) > 180) throw new Error(`Nexus could not locate ${requested} for fallback weather.`);
    ...
    if (!Number.isFinite(details.air_temperature) || !point?.time) throw new Error("Fallback weather returned no current observation.");
    ...
    return Object.freeze({
      ...
      precipitationMm: next?.details?.precipitation_amount ?? null,
      weatherCode: null,
      windKph: typeof details.wind_speed === "number" ? Math.round(details.wind_speed * 3.6) : null,
      highC: null,
      lowC: null,
      rainChance: next?.details?.probability_of_precipitation ?? null,
  ...
  async function retrieveWeather(requested) {
    ...
    if (!place || !Number.isFinite(place.latitude) || !Number.isFinite(place.longitude) || Math.abs(place.latitude) > 90 || Math.abs(place.longitude) > 180) throw new Error(`Nexus could not locate ${requested} for weather.`);
    ...
    if (!Number.isFinite(forecast?.current?.temperature_2m) || !forecast?.current?.time) throw new Error("Weather returned no current observation.");
    return Object.freeze({
  ...
  const service = {
    async weather(command) {
      ...
      if (cached && cacheAgeMs >= 0 && cacheAgeMs <= Math.min(600000, Math.max(0, weatherCacheTtlMs))) return cached.value;
      ...
      const request = retrieveWeather(requested).catch(() => retrieveMetNorwayWeather(requested)).then((weather) => {
        ...
        if (cached && cacheAgeMs >= 0 && cacheAgeMs <= Math.min(3600000, Math.max(0, weatherStaleTtlMs))) {
