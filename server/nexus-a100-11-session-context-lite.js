...
const SPRINT_ID = "A100-11";
const SPRINT_SLUG = "session-context-lite";
const SPRINT_TITLE = "Session Context-Lite";
const SCHEMA_VERSION = "nexus.a100.11.session-context-lite.v1";
...
function classifyA100Sprint11Lane(prompt = "") {
  const lower = text(prompt).toLowerCase();
  if (lower.includes("topic")) return "topic";
  if (lower.includes("lastrequest")) return "lastRequest";
  if (lower.includes("locale")) return "locale";
  if (lower.includes("mode")) return "mode";
  if (lower.includes("expires")) return "expires";
  return SUPPORTED_LANES[0];
}

function createA100Sprint11Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare Session Context-Lite.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint11Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-11-session-context-lite`, `${lane}:${prompt}`)),
    artifactType: "session_context_lite_review_packet",
    lane,
    prompt,
    objective: "Carries short-lived context while preventing memory from becoming execution authority.",
    status: "prepared_for_review",
    userFacingSummary: `Session Context-Lite: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint11Artifact(artifact) {
