...
const SPRINT_ID = "A100-15";
const SPRINT_SLUG = "qa-hardening";
const SPRINT_TITLE = "QA Hardening";
const SCHEMA_VERSION = "nexus.a100.15.qa-hardening.v1";
...
function classifyA100Sprint15Lane(prompt = "") {
  const lower = text(prompt).toLowerCase();
  if (lower.includes("static")) return "static";
  if (lower.includes("runtime")) return "runtime";
  if (lower.includes("secrets")) return "secrets";
  if (lower.includes("execution")) return "execution";
  if (lower.includes("drift")) return "drift";
  return SUPPORTED_LANES[0];
}

function createA100Sprint15Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare QA Hardening.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint15Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-15-qa-hardening`, `${lane}:${prompt}`)),
    artifactType: "qa_hardening_review_packet",
    lane,
    prompt,
    objective: "Adds static QA expectations for no secrets, no uncontrolled execution, and no runtime activation drift.",
    status: "prepared_for_review",
    userFacingSummary: `QA Hardening: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint15Artifact(artifact) {
