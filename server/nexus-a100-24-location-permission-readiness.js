...
const SPRINT_ID = "A100-24";
const SPRINT_SLUG = "location-permission-readiness";
const SPRINT_TITLE = "Location Permission Readiness";
const SCHEMA_VERSION = "nexus.a100.24.location-permission-readiness.v1";
...
function classifyA100Sprint24Lane(prompt = "") {
...
function createA100Sprint24Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare Location Permission Readiness.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint24Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-24-location-permission-readiness`, `${lane}:${prompt}`)),
    artifactType: "location_permission_readiness_review_packet",
    lane,
    prompt,
    objective: "Explains location requirements without prompting browser permission or starting tracking.",
    status: "prepared_for_review",
    userFacingSummary: `Location Permission Readiness: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint24Artifact(artifact) {
