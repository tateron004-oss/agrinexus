...
function selectIntent(goal = "") {
  ...
  if (hasAny(text, [/remind|reminder|check.*later/]) && !hasAny(text, [/\b(send|provider|readings|care team|doctor|clinician)\b/])) return { domain: "workflow", intent: "create_reminder", capabilityIds: ["workflow.reminder"], missingInformation: /tonight|tomorrow|later|today|next|morning|afternoon|evening|at \d/i.test(text) ? [] : ["reminder timing"], riskLevel: "low" };
  ...
  if (hasAny(text, [/blood pressure|bp\b|hypertension|htn\b|diabetes|diabetic|dm\b|glucose|a1c|obesity|weight|bmi|reading|rpm|rtm|remote patient|remote therapeutic/])) return { domain: "medical", intent: "chronic_care_provider_review", capabilityIds: ["medical.chronicCare", "medical.rpm", "medical.rtm", "medical.telehealthProvider"], missingInformation: /blood pressure|bp|hypertension|htn|diabetes|diabetic|dm\b|glucose|a1c|obesity|weight|bmi|reading|rpm|rtm/.test(text) && !/\d/.test(text) ? ["recent RPM/RTM readings or participation context with date and time"] : [], riskLevel: "moderate" };
  ...
  if (hasAny(text, [/\b(weather|forecast|temperature|heat risk|rain|raining|climate)\b/])) return { domain: "knowledge", intent: "weather_lookup", capabilityIds: ["knowledge.weather"], missingInformation: /\b(in|for|at|near)\s+[a-z0-9 ,.-]+/i.test(text) ? [] : ["explicit city, country, or region"], riskLevel: "low" };
  ...
  if (hasAny(text, [/agriculture training|irrigation|crop|farmer training|tomato|blight|pest|disease|soil|planting|harvest|farm/])) return { domain: "agriculture", intent: "agriculture_learning", capabilityIds: ["agriculture.learning"], missingInformation: [], riskLevel: "low" };
  ...
  if (hasAny(text, [/current|latest|live web|internet|search|source|citation|cite|research|best practices|today/])) return { domain: "knowledge", intent: "current_information_lookup", capabilityIds: ["knowledge.liveWeb"], missingInformation: [], riskLevel: "low" };
