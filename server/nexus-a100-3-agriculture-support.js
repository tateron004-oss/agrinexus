...
const SPRINT_ID = "A100-3";
const SPRINT_SLUG = "agriculture-support";
const SPRINT_TITLE = "Agriculture Support";
const SCHEMA_VERSION = "nexus.a100.3.agriculture-support.v1";
...
function classifyA100Sprint3Lane(prompt = "") {
...
function createA100Sprint3Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare Agriculture Support.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint3Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-3-agriculture-support`, `${lane}:${prompt}`)),
    artifactType: "agriculture_support_review_packet",
    lane,
    prompt,
    objective: "Structures crop, pest, soil, weather, and extension questions into citation-ready advisory previews.",
    status: "prepared_for_review",
    userFacingSummary: `Agriculture Support: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint3Artifact(artifact) {
