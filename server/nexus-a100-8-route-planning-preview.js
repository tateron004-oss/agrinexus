...
const SPRINT_ID = "A100-8";
const SPRINT_SLUG = "route-planning-preview";
const SPRINT_TITLE = "Route Planning Preview";
const SCHEMA_VERSION = "nexus.a100.8.route-planning-preview.v1";
...
function classifyA100Sprint8Lane(prompt = "") {
...
function createA100Sprint8Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare Route Planning Preview.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint8Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-8-route-planning-preview`, `${lane}:${prompt}`)),
    artifactType: "route_planning_preview_review_packet",
    lane,
    prompt,
    objective: "Prepares review-only route intent without geolocation, navigation, or provider handoff.",
    status: "prepared_for_review",
    userFacingSummary: `Route Planning Preview: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint8Artifact(artifact) {
