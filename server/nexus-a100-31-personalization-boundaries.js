...
const SPRINT_ID = "A100-31";
const SPRINT_SLUG = "personalization-boundaries";
const SPRINT_TITLE = "Personalization Boundaries";
const SCHEMA_VERSION = "nexus.a100.31.personalization-boundaries.v1";
...
function classifyA100Sprint31Lane(prompt = "") {
  ...
  if (lower.includes("forget")) return "forget";
  return SUPPORTED_LANES[0];
}

function createA100Sprint31Artifact(input = {}) {
  const prompt = text(input.prompt, "Prepare Personalization Boundaries.");
  const lane = SUPPORTED_LANES.includes(input.lane) ? input.lane : classifyA100Sprint31Lane(prompt);
  return Object.freeze({
    schemaVersion: SCHEMA_VERSION,
    sprintId: SPRINT_ID,
    sprintSlug: SPRINT_SLUG,
    sprintTitle: SPRINT_TITLE,
    artifactId: text(input.artifactId, stableId(`a100-31-personalization-boundaries`, `${lane}:${prompt}`)),
    artifactType: "personalization_boundaries_review_packet",
    lane,
    prompt,
    objective: "Uses preferences as hints while blocking authority, identity, payment, and medical assumptions.",
    status: "prepared_for_review",
    userFacingSummary: `Personalization Boundaries: Nexus can prepare a safe, review-only packet for ${lane}. It cannot execute real-world actions from this sprint.`,
...
function isSafeA100Sprint31Artifact(artifact) {
