...

  {
    ...
    const service = createVisualDataService({
      ...
      fetchImpl: async (url) => {
        ...
        calls.push(target);
        if (target.includes("open-meteo.com")) return { ok: false, status: 429, headers: { get: () => "0" }, body: { cancel: async () => {} } };
        if (target.includes("nominatim.openstreetmap.org")) return { ok: true, status: 200, headers: { get: () => null }, async json() { return [{ display_name: "Nairobi, Kenya", lat: "-1.2864", lon: "36.8172" }]; } };
        if (target.includes("api.met.no")) return { ok: true, status: 200, headers: { get: () => null }, async json() { return { properties: { timeseries: [{ time: "2026-08-05T12:00:00Z", data: { instant: { details: { air_temperature: 24, wind_speed: 3 } }, next_1_hours: { summary: { symbol_code: "partly_cloudy_day" }, details: { precipitation_amount: 0, probability_of_precipitation: 10 } } } }] } }; } };
        throw new Error(`unexpected weather URL ${target}`);
      }
    });
    const weather = await service.weather("Show today's weather in Nairobi, Kenya");
    assert.equal(weather.status, "live-weather-fallback-ready");
    assert.equal(weather.sourceName, "MET Norway Locationforecast");
    assert.equal(weather.temperatureC, 24);
    assert(calls.some(url => url.includes("nominatim.openstreetmap.org")), "Open-Meteo 429 must change geocoding providers");
    assert(calls.some(url => url.includes("api.met.no")), "Open-Meteo 429 must change forecast providers");
  }
