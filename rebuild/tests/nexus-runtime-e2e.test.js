...
async function main() {
  ...
  assert.ok(receipts.some((receipt) => receipt.type === "audio.exclusive-owner-violation"));
  assert.ok(sent.some((event) => event.type === "response.cancel" && event.response_id === "response-overlap"));
  await runtime.handleRealtimeEvent({ type: "input_audio_buffer.speech_started" });
