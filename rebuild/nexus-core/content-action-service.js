...
function createContentActionService({ fetchImpl = globalThis.fetch, musicProvider = null, goalResolver = null, explanationResolver = null, webSearchProvider = null, publicMusicProvider = true } = {}) {
  ...
  async function research(goal, context) {
    ...
    let live;
    try {
      live = await liveWebSearch(query);
    } catch (primaryError) {
      ...
      const response = await providerFetch(url, { headers: { "user-agent": "Nexus-Genesis/1.0 (source-search-fallback)" } });
      if (!response.ok) throw primaryError;
      const payload = await response.json();
      const results = Array.isArray(payload && payload.query && payload.query.search) ? payload.query.search : [];
      if (!results.length) throw primaryError;
      live = {
        summary: "The primary live-search provider was temporarily unavailable. Nexus recovered with current, source-linked reference results.",
