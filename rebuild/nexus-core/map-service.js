...
function createOpenMapProvider({ fetchImpl = fetch } = {}) {
  ...
  async function photonGeocode(query) {
    ...
    if (!feature || !Array.isArray(coordinates) || coordinates.length < 2) throw new Error(`Nexus could not locate ${query} with the fallback map provider.`);
    const properties = feature.properties || {};
    return [{
      display_name: [properties.name, properties.city, properties.state, properties.country].filter(Boolean).filter((value, index, all) => all.indexOf(value) === index).join(", ") || query,
      lat: coordinates[1],
      lon: coordinates[0],
      boundingbox: [],
      addresstype: properties.type || "administrative",
      category: "place",
      address: { country_code: String(properties.countrycode || "").toLowerCase() }
    }];
  }
