...
function countryCodeFromQuery(query) {
  ...
  const country = Object.keys(COUNTRY_CODES).find(name => {
...
function createOpenMapProvider({ fetchImpl = fetch, sleepImpl } = {}) {
  const providerFetch = createProviderFetch({ fetchImpl, sleepImpl });
  async function photonGeocode(query) {
    const parameters = new URLSearchParams({ q: query, limit: "5" });
    const response = await providerFetch(`https://photon.komoot.io/api/?${parameters.toString()}`, {
      headers: { "user-agent": "Nexus-Genesis/1.0 (map-fallback)" }
    });
    if (!response.ok) throw new Error(`Fallback map location lookup failed (${response.status}).`);
    const payload = await response.json();
    return (Array.isArray(payload && payload.features) ? payload.features : []).filter(feature =>
      ...
      return {
        display_name: [properties.name, properties.city, properties.state, properties.country].filter(Boolean).join(", ") || query,
        lat: coordinates[1], lon: coordinates[0], boundingbox: [], addresstype: properties.type || "administrative",
        category: "place", address: { country_code: String(properties.countrycode || "").toLowerCase() }
      };
    });
  }
  ...
  async function geocode(query) {
    ...
    try {
      ...
      results = await photonGeocode(normalizedQuery).catch(() => { throw error; });
    }
  ...
  return async function resolve(command) {
    ...
    for (const routeUrl of routeUrls) {
      try {
        const response = await providerFetch(routeUrl, { headers: { "user-agent": "Nexus-Genesis/1.0 (route-support)" } });
